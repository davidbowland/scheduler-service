...
describe('Test for sqs-payload-logger', function () {
  ...
  it('Verifies the payload is logged', async () => {
    ...
    await scheduledEventLoggerHandler(payload, context)

    // Verify that console.info has been called with the expected payload
    expect(console.info).toHaveBeenCalledWith('event', JSON.stringify(payload))
    expect(console.info).toHaveBeenCalledWith('context', JSON.stringify(context))
  })
})
